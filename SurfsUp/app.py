...
def precipitation():
    ...
    format = '%Y-%m-%d'
    most_recent_date = dt.datetime.strptime(most_recent_date[0], format).date()
...
def temperature_observations():
    ...
    popular_station_id = session.query((Measurement.station.distinct()),func.count(Measurement.station)).\
    ...
    filter(Measurement.date >= last_year).\
        filter(Measurement.station == popular_station_id).all()
    session.close()
...
def start_date(start_date):
    ...
    if date1 > most_recent_date:
        session.close()
        return jsonify({"error": f"The most recent date with available data is '{most_recent_date}'. Try with an earlier date"}), 404
...
def start_end_date(start_date,end_date):
    ...
    if date1 > most_recent_date:
        session.close()
        return jsonify({"error": f"The most recent date with available data is '{most_recent_date}'. Try with a Start Date equal to earlier than that"}), 404
